...
/*
 解析行程摘要数据
 */
+ (void)parseDriveSummary:(NSString*) data
{ 
...
/*
 解析行程切片数据
 */
+ (LCDrivePiece*)parseDrivePiece:(NSString*)data withLevel:(int) level
{
    ...
    int hour = 0, minute = 0, second = 0;
    double lat = 0.0, lng = 0.0;
    int ele = 0, dist = 0, fuel = 0, avgFeul = 0, avgRPM = 0, maxRPM = 0;
    NSRange range;
    range.length = 2;
    for(int i=0; i<[data length]; i+=2)
    {
    ...
    double score = [self score:drivePiece withLevel:level];
    drivePiece.score = score;
    return drivePiece;
}
